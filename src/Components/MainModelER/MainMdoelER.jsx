...
export default function MainMdoelER() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("er_chats");
    if (saved) {
  ...
  useEffect(() => {
    if (chats.length > 0) {
      localStorage.setItem("er_chats", JSON.stringify(chats));
    }
  }, [chats]);
  ...
  const deleteChat = (e, chatId) => {
    ...
    setChats(updated);
    localStorage.setItem("er_chats", JSON.stringify(updated));
